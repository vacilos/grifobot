...
<div class="container">
    <div class="row">
        <div class="col-sm-12">
            ...
            @endif
            <form method="post" action="{{ route('quiz_update_math', ['quiz'=>$quiz, 'math'=>$math]) }}" id="mathForm" enctype="multipart/form-data">
                @csrf
                @method('patch')
                ...
                <button type="submit" class="btn btn-success">Υποβολή</button> <a href="{{route('quiz_questions', ['quiz'=>$quiz->id])}}" class="btn btn-default">Επιστροφή</a>
            </form>
        </div>
    </div>
</div>
